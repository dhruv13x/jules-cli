...
def test_init_command_creates_config(tmp_path):
    ...
    # Mock setup_logging to avoid "I/O operation on closed file"
    with patch("jules_cli.utils.config.DEFAULT_CONFIG_PATH", str(config_path)), \
         ...
         patch("jules_cli.utils.logging.setup_logging") as mock_logging:
         ...
         assert "Jules Initialization Wizard" in result.stdout
