...
@patch('src.jules_cli.commands.pr.logger')
def test_cmd_create_pr_no_token(mock_logger):
    with patch('src.jules_cli.commands.pr.GITHUB_TOKEN', None):
        pr.cmd_create_pr()
        mock_logger.error.assert_called_with("GITHUB_TOKEN not set; cannot create PR.")

@patch('src.jules_cli.commands.pr.logger')
def test_cmd_create_pr_no_repo(mock_logger):
    with patch('src.jules_cli.commands.pr.GITHUB_TOKEN', 'test_token'):
        _state["repo_owner"] = None
        pr.cmd_create_pr()
        mock_logger.warning.assert_called_with("No repo detected in state. Run a task first.")

@patch('src.jules_cli.commands.pr.git_current_branch', return_value="my-branch")
@patch('src.jules_cli.commands.pr.github_create_pr', side_effect=Exception("API error"))
@patch('src.jules_cli.commands.pr.logger')
